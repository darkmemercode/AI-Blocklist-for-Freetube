...
- `fetch_channels_faster.py` generates the `freetube_channels.json` file needed for the database update with `update_db.py` and it creates `handles_mapping.json` that acts like a cache to speed up reruns when updating the blocklist.
